...
async function fetchRecommendations() {
  ...
  // Use user agent and timestamp as seed for deterministic "randomness"
  const seed = userAgent.length + Date.now();

  // Generate recommendations using deterministic values
  return Array.from({ length: 4 }, (_, i) => ({
    id: i + 1,
    title: `Recommended Product ${i + 1}`,
    category: categories[(seed + i) % categories.length],
    reason: reasons[(seed + i * 2) % reasons.length],
    price: ((seed + i * 10) % 200) + 20,
    rating: (((seed + i * 3) % 20) / 10 + 3).toFixed(1), // 3.0 - 5.0
    discount: ((seed + i * 5) % 30) + 10, // 10-40% off
  }));
}
      ...
      <div className="mt-6 pt-4 border-t">
        <div className="flex items-center justify-between">
          <p className="text-xs text-gray-500">
            🤖 Generated by ML algorithms in real-time
          </p>
