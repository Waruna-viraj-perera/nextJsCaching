...
async function fetchUserData() {
  // Access request data to ensure this is properly dynamic
  ...
  // Use the user agent or timestamp as a seed for deterministic "randomness"
  const seed = userAgent.length + Date.now();
  const orderCount = (seed % 50) + 1;
  ...
  return {
    ...
    lastVisit: new Date().toISOString().slice(0, 19).replace("T", " "), // More consistent format
    location: "San Francisco, CA",
    memberSince: "January 2024",
    orderCount: orderCount,
    favoriteCategory: ["electronics", "accessories", "books"][categoryIndex],
  };
}
